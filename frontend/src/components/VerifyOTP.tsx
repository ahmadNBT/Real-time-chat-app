...
const VerifyOTP = () => {
  ...
  const [resedLoading, setResedLoading] = useState(false);
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLElement>): void => {
    ...
    const digits: any = pastedData.replace(/\D/g, "").slice(0, 6);
    if (digits.length === 6) {
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-8">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              ...
              <div className="flex justify-center items-center in-checked: space-x-3">
                {otp.map((digit, index) => (
                  <input
                    key={index}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-4 px-6 rounded-lg flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
